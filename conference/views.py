        ...
        if len(phone_number) != 0:
            for ph in phone_number:
                ...
                if key:
                    ...
                    payload = {'msisdn': phone, 'text': f"https://vconf.pager.uz/#/conference-live/{conf.id} sizni konferentsiyaga taklif qiladi:{user_id.username},boshlanish vaqti: {when}{start_time}, kirish uchun KOD: %s" % key, 'priority':"1", 'id': 0,'delivery-notification-requested' : 'true','login' : settings.SMS_LOGIN, 'password': settings.SMS_PASSWORD, 'ref-id': 0,'version':1.0}
